...
},
{
	type:'input',
	name:'sleep',
	message:'[-X-] Insert Sleep (MiliSeconds):',
	validate: function(value){
		value = value.match(/[0-9]/);
		if (value) return true;
...
const Excute = async function(User,sleep,mysyntx){
	try {
		console.log(chalk`\n{white [-X-] Try to Login . . .}`);
		const doLogin = await Login(User);
		console.log(chalk`{green [-X-] Login Success}, {lime [>] Synchronizing Data . . .}`);
		const task = [
		...
		console.log(chalk`{red  | Account To Unfollow : ${AccountToUnfollow.length}}`)
		...
		for (let i = 0; i < AccountToUnfollow.length; i++) {
			...
			await Promise.all(AccountToUnfollow[i].map(async(akun) => {
				...
				console.log(chalk`[{cyan ${timeNow}}] [-X-] Unfollow {cyan [${akun}]} => ${doUnfollow}`);
			}))
			await console.log(chalk`{red \n [-X-] Sleep ${sleep} MiliSeconds \n}`);
			await delay(sleep);
		}
	} catch(err) {
		console.log(err);
...
console.log(chalk`
  ...
  [-X-] {bold.green Unfollow Not Followback New Version!}
